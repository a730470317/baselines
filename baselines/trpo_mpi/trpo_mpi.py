...
def learn(*,
    ...
    **network_kwargs        keyword arguments to the policy / network builder. See baselines.common/policies.py/build_policy and arguments to a particular type of network
    ...
    mean_kl = tf.reduce_mean(kld_old_new)
    ...
    iters_so_far = 0
    tstart = time.time()
    length_buffer = deque(maxlen=40)  # rolling buffer for episode lengths
    ...
    while True:
        ...
        args = seg["ob"], seg["ac"], target_advantage_function
        ...
        length_buffer.extend(lens)
        reward_buffer.extend(rews)

        logger.record_tabular("EpLenMean", np.mean(length_buffer))
        ...
        try:
            ...
            if (iters_so_far >= 10 and
                ...
                pi.save(file_name)
                f = open(file_name+str(int(np.mean(reward_buffer))),'w')
                f.close()
        except Exception as e:
